...
//----------------------------------------------------------
// modified by @trapias for TrelloExport
...
function xlsx(file) { 
	...
	/* @trapias: modified to strip more chars to avoid errors opening file in Excel */
	function escapeXML(s) { return typeof s === 'string' ? s.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/'/g, '&#x27;').replace(/\u00B7/g,'').replace(/\u00C2/g,'').replace(/\u00A0/g,'').replace(/\u00A2/g,'').replace(/\u00A3/g,'').replace(/[^\u000D\u000A\u00B7\u0020-\u007E]/g,'') : ''; }
